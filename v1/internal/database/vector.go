...
func (v *VectorDatabase) DeletePointsByFilter(collectionName string, filter map[string]interface{}) error {
	// Создаем фильтр в формате Qdrant
	...
	for key, value := range filter {
		...
		switch v := value.(type) {
		case string:
			condition = qdrant.NewMatch(key, v)
		case int:
			condition = qdrant.NewMatch(key, strconv.FormatInt(int64(v), 10))
		case int64:
			condition = qdrant.NewMatch(key, strconv.FormatInt(v, 10))
		case float64:
...
func (v *VectorDatabase) Search(collectionName string, queryVector []float64, limit int, scoreThreshold float32) ([]SearchResult, error) {
	...
	// Use Query method with proper vector parameter
	searchResult, err := v.client.Query(context.Background(), &qdrant.QueryPoints{
		...
		WithVectors:    qdrant.NewWithVectors(false), // Don't return vectors to save bandwidth
	})
	...
	for i, point := range searchResult {
		...
		// Extract payload from Qdrant response
		if point.Payload != nil {
			for key, value := range point.Payload {
				...
				case *qdrant.Value_ListValue:
					// Handle list values if needed
					payload[key] = v.ListValue
				case *qdrant.Value_StructValue:
					// Handle struct values if needed
					payload[key] = v.StructValue
				}
			}
		}
...
func (v *VectorDatabase) SearchWithFilter(collectionName string, queryVector []float64, limit int, scoreThreshold float32, filter map[string]interface{}) ([]SearchResult, error) {
	queryVector32 := make([]float32, len(queryVector))
	for i, val := range queryVector {
		queryVector32[i] = float32(val)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to search with filter in collection %s: %w", collectionName, err)
	}
