...
import (
	"fmt"
	database2 "macdent-ai-chatbot/v1/internal/database"
...
type KnowledgeService struct {
	...
	vectorDB *database2.VectorDatabase
}
...
func NewKnowledgeService(db *database2.Database, vectorDB *database2.VectorDatabase) *KnowledgeService {
...
func (k *KnowledgeService) AddDocument(agentID uuid.UUID, fileName string, content string, fileSize int64, fileType string) error {
	...
	points := make([]database2.VectorPoint, len(chunks))
	for i, chunk := range chunks {
		points[i] = database2.VectorPoint{
			ID:      chunk.ID,
			Vector:  embeddings[i],
